...
//             // Nếu dùng PHP session, nên gọi route logout của bạn:
...
//         const ngay = $('#ngay-tao');
//         if (ngay) {
//             // ưu tiên valueAsDate nếu hỗ trợ
//             if ('valueAsDate' in ngay) ngay.valueAsDate = new Date();
//             else ngay.value = new Date().toISOString().split('T')[0];
//         }
//         openModal(modalND);
//     });
...
//             const next = (cur === 'Hoạt động') ? 'Vô hiệu hóa' : 'Hoạt động';
//             row.cells[4].textContent = next;
//             btn.textContent = (next === 'Hoạt động') ? 'Vô hiệu' : 'Kích hoạt';
...
//     (function initPerm() {
//         const role = $('#chon-vai-tro') ?.value || 'Giáo viên';
...
//         a.href = url;
//         a.download = 'nhat-ky-he-thong.csv';
...
//             const okDM = dm ? _dm.toLowerCase().includes(dm.toLowerCase()) : true;
...
//         const tkten = $('#tk-ten');
//         if (tkten) tkten.value = '';
//         const tkdm = $('#tk-danh-muc');
//         if (tkdm) tkdm.value = '';
//         const tktt = $('#tk-tinh-trang');
//         if (tktt) tktt.value = '';
//         const tklop = $('#tk-lop');
//         if (tklop) tklop.value = '';
...
// //modal sửa: gửi ajax và mở modal
// document.querySelectorAll('.nut-sua').forEach(btn => {
//   btn.addEventListener('click', () => {
//     const maND = btn.dataset.mand;

//     fetch(`controllers/TaikhoanController.php?action=getUser&mand=${maND}`)
//       .then(res => res.json())
//       .then(data => {
//         // Gán dữ liệu vào form sửa
//         document.getElementById('sua-maND').value = data.MaND;
//         document.getElementById('sua-hoTen').value = data.hoTen;
//         document.getElementById('sua-email').value = data.email;
//         document.getElementById('sua-vaitro').value = data.maVT;
//         document.getElementById('sua-trangthai').value = data.trangthai;
//         document.getElementById('sua-username').value = data.username;
//         document.getElementById('sua-ngayTao').value = data.ngayTao;
...
// //modal sửa: đóng
// const modalSua = document.getElementById('modal-sua-nguoi-dung');
// document.getElementById('dong-modal-sua').addEventListener('click', () => {
//   modalSua.classList.remove('open');
//   document.body.classList.remove('no-scroll');
// });
// document.getElementById('huy-sua').addEventListener('click', () => {
//   modalSua.classList.remove('open');
//   document.body.classList.remove('no-scroll');
// });
