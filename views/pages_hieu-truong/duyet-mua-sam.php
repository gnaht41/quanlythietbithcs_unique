...
<section id="duyet-mua-sam" class="trang-an"
  <?php echo ($active_tab != 'duyet-mua-sam') ? 'style="display:none;"' : ''; ?>>
  ...
  <style>
    ...
    #duyet-mua-sam .filter input,
    #duyet-mua-sam .filter select {
      padding: 8px 10px;
    ...
    #duyet-mua-sam tbody tr:hover {
      background: #f9fafb
    }

    #duyet-mua-sam .col-stt {
      width: 54px;
      text-align: center
    }

    #duyet-mua-sam .btn-action {
      background: #e5e7eb;
  ...
  <table>
    ...
    <tbody>
      <?php if (empty($list)): ?>
        <tr>
          <td colspan="6" style="text-align:center;padding:16px;color:#6b7280;">Chưa có kế hoạch mua sắm.</td>
        </tr>
      <?php else: ?>
        <?php $stt = 1;
        foreach ($list as $row): ?>
  ...
  <div id="msModal">
    <div id="msBox">
      ...
      <div class="card"
        style="background:#fff;border-radius:10px;padding:14px;box-shadow:0 2px 10px rgba(0,0,0,.06);">
        <form method="GET" style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
          ...
          <select name="status"
            style="height:36px;padding:0 10px;border:1px solid #d0d7de;border-radius:8px;outline:none;">
            <option value="">-- Trạng thái --</option>
            <option value="Chờ duyệt" <?php echo ($status === 'Chờ duyệt') ? 'selected' : ''; ?>>Chờ duyệt
            </option>
            <option value="Đã duyệt" <?php echo ($status === 'Đã duyệt') ? 'selected' : ''; ?>>Đã duyệt
            </option>
            <option value="Từ chối" <?php echo ($status === 'Từ chối') ? 'selected' : ''; ?>>Từ chối
            </option>
          </select>

          <button type="submit" class="btn"
            style="height:36px;border-radius:8px;padding:0 14px;border:0;background:#16a34a;color:#fff;">Lọc</button>
          <a href="?tab=duyet-mua-sam" style="color:#2563eb;text-decoration:none;">Xóa lọc</a>
        </form>
      </div>

      <div class="card"
        style="margin-top:14px;background:#fff;border-radius:10px;padding:0;box-shadow:0 2px 10px rgba(0,0,0,.06);overflow:hidden;">
      ...
      <div id="msModalOverlay"
        style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:999;opacity:0;transition:opacity .18s ease;">
        <div id="msModal"
          style="width:820px;max-width:92vw;background:#fff;border-radius:12px;position:absolute;left:50%;top:50%;transform:translate(-50%,-48%);box-shadow:0 10px 30px rgba(0,0,0,.22);padding:18px;opacity:0;transition:opacity .18s ease, transform .18s ease;">
          <div style="display:flex;justify-content:space-between;align-items:flex-start;">
            <div>
              <h3 style="margin:0 0 8px 0;">Chi tiết kế hoạch mua sắm</h3>
            </div>
            <button type="button" onclick="closeMSModal()"
              style="border:0;background:#f1f5f9;border-radius:8px;width:32px;height:32px;cursor:pointer;">×</button>
          </div>

          <div id="msDetailBody"
            style="display:grid;grid-template-columns:1fr 1fr;gap:10px 24px;margin-top:6px;color:#0f172a;">
            <div>
          ...
          <div>
            ...
            <select id="ms_decision"
              style="width:100%;height:38px;border:1px solid #d0d7de;border-radius:8px;padding:0 10px;outline:none;">
              <option value="approve">Phê duyệt</option>
              <option value="reject">Từ chối</option>
            </select>
          </div>
          ...
          <div style="display:flex;gap:10px;align-items:center;margin-top:14px;">
            <button type="button" onclick="submitMSDecision()"
              style="height:34px;border-radius:8px;padding:0 14px;border:0;background:#16a34a;color:#fff;cursor:pointer;">
              Lưu
            </button>
            <button type="button" onclick="closeMSModal()"
              style="height:34px;border-radius:8px;padding:0 14px;border:1px solid #d1d5db;background:#f3f4f6;cursor:pointer;">
              Hủy
            </button>
      ...
      <script>
        ...
        async function openMSDetail(maMS) {
          ...
          try {
            const res = await fetch(`${AJAX_URL_MS}?ajax=detail&maMS=${maMS}`, {
