...
<table><tr><th>STT</th><th>Mã</th><th>Thiết bị</th><th>SL</th><th>Mục đích</th><th>Ngày mượn</th><th>Ngày trả</th><th>Trạng thái</th><th>Thao tác</th></tr><tbody id="list"></tbody></table>

<div id="modal" class="modal"><div class="modal-content"><h3 id="modal-title">Tạo phiếu mượn</h3><form onsubmit="save(event)">
<label>Ngày mượn:</label><input type="date" id="ngaymuon" required>
<label>Ngày trả:</label><input type="date" id="ngaytra" required>
<label>Mục đích:</label><select id="mucdich" required><option value="">Chọn mục đích</option><option value="Dạy học">Dạy học</option><option value="Họp phụ huynh">Họp phụ huynh</option><option value="Hội nghị">Hội nghị</option><option value="Thi cử">Thi cử</option><option value="Hoạt động ngoại khóa">Hoạt động ngoại khóa</option></select>
<label>Thiết bị:</label><div id="equipment-list">
<div class="equipment"><input type="checkbox" onchange="toggleEquipment(this,'Máy tính Dell','TB-001')"> Máy tính Dell (TB-001) - SL: <input type="number" min="1" max="5" value="1" onclick="event.stopPropagation()" onchange="updateSelected()"></div>
<div class="equipment"><input type="checkbox" onchange="toggleEquipment(this,'Máy chiếu Epson','TB-002')"> Máy chiếu Epson (TB-002) - SL: <input type="number" min="1" max="3" value="1" onclick="event.stopPropagation()" onchange="updateSelected()"></div>
<div class="equipment"><input type="checkbox" onchange="toggleEquipment(this,'Loa Bluetooth','TB-003')"> Loa Bluetooth (TB-003) - SL: <input type="number" min="1" max="10" value="1" onclick="event.stopPropagation()" onchange="updateSelected()"></div>
...
<button type="submit" class="btn btn-primary">Lưu</button><button type="button" onclick="closeModal()" class="btn btn-danger">Hủy</button>
</form></div></div>

<div id="modal-detail" class="modal"><div class="modal-content"><h3>Chi tiết phiếu</h3><div id="detail-content"></div><button onclick="closeModal('modal-detail')" class="btn btn-primary">Đóng</button></div></div>
...
const equipmentText = item.thietbi;let equipmentName = equipmentText;let totalQuantity = 0;
const quantityMatches = equipmentText.match(/SL:(\d+)/g);
if (quantityMatches) {totalQuantity = quantityMatches.reduce((sum, match) => sum + parseInt(match.replace('SL:', '')), 0);equipmentName = equipmentText.replace(/\s*SL:\d+/g, '');}
...
}).join('');} else {list.innerHTML = '<tr><td colspan="9">Chưa có phiếu</td></tr>';}});
}
function openCreate() {
    editId = null;
...
function toggleEquipment(checkbox, name, code) {const el = checkbox.closest('.equipment');el.classList.toggle('selected', checkbox.checked);updateSelected();}
function updateSelected() {const selected = [];document.querySelectorAll('.equipment').forEach(el => {const cb = el.querySelector('input[type="checkbox"]');const qty = el.querySelector('input[type="number"]');if (cb.checked) {const text = el.textContent.split(' - SL:')[0].replace('☐', '').replace('☑', '').trim();selected.push(`${text} SL:${qty.value}`);}});document.getElementById('selected-equipment').value = selected.join(', ');}
function clearEquipment() {document.querySelectorAll('.equipment').forEach(el => {el.querySelector('input[type="checkbox"]').checked = false;el.querySelector('input[type="number"]').value = 1;el.classList.remove('selected');});document.getElementById('selected-equipment').value = '';}
function view(id) {fetch(API + '?action=detail&id=' + id).then(r => r.json()).then(data => {if (data.success) {const item = data.data;document.getElementById('detail-content').innerHTML = `<p><b>Mã:</b> ${item.ma}</p><p><b>Thiết bị:</b> ${item.thietbi}</p><p><b>Mục đích:</b> ${item.mucdich}</p><p><b>Ngày mượn:</b> ${formatDate(item.ngaymuon)}</p><p><b>Ngày trả:</b> ${formatDate(item.ngaytra)}</p><p><b>Trạng thái:</b> <span class="status-text">${item.trangthai}</span></p>`;document.getElementById('modal-detail').style.display = 'block';} else alert('Lỗi: ' + data.message);});}
function edit(id) {fetch(API + '?action=detail&id=' + id).then(r => r.json()).then(data => {if (data.success) {const item = data.data;editId = id;document.getElementById('modal-title').textContent = 'Sửa phiếu mượn';document.getElementById('ngaymuon').value = item.ngaymuon;document.getElementById('ngaytra').value = item.ngaytra;document.getElementById('mucdich').value = item.mucdich || '';clearEquipment();if (item.thietbi) document.getElementById('selected-equipment').value = item.thietbi;document.getElementById('diadiem').value = item.diadiem || '';document.getElementById('ghichu').value = item.ghichu || '';document.getElementById('modal').style.display = 'block';} else alert('Lỗi: ' + data.message);});}
...
function save(e) {
    ...
    fetch(API, {method: 'POST', body: form}).then(r => r.json()).then(result => {
...
function del(id) {if (confirm('Xóa phiếu mượn này?')) {fetch(API, {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({action: 'delete', id: id})}).then(r => r.json()).then(data => {alert(data.success ? 'Xóa thành công' : 'Lỗi: ' + data.message);if (data.success) loadList();});}}

function closeModal(modalId = 'modal') {document.getElementById(modalId).style.display = 'none';}
