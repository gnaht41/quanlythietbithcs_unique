...

{
    ...
    public function themtaikhoan($maND, $hoten, $email, $maVT, $pass, $trangthai)
    {
        ...
        $sqltaikhoan = "INSERT INTO taikhoan (username, password, maND, trangThai)
                        VALUES ('$email','$pass', '$maND', '$trangthai');";
    ...
    {
        $sql = "UPDATE taikhoan SET trangThai = 'Vô hiệu hóa' WHERE maND = '$maND'";
        if($this->sql($sql) == 1)
        {
            header("Location: " . $_SERVER['REQUEST_URI']);
        }
    ...
    public function SuaTK($maND, $hoTen, $maVT, $trangthai, $password, $passwordmoi, $passwordnhaplai)
    {
        ...
        if($password != "")
        {
            ...
            if($password == $taikhoan['password'])
            {
                ...
                {
                    $sql1 = "update taikhoan set password = '$passwordmoi'";
                    $updatePass = $this->sql($sql1);
                }
            }
        }

        $sql2 = "update nguoidung set hoTen = '$hoTen', maVT = $maVT 
                                  where maND = '$maND'";
