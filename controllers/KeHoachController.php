<?php
// controllers/KeHoachController.php
session_start();
...
$model = new KeHoach($db);

// Chỉ Hiệu trưởng (maVT = 2)
...
try {
    switch ($action) {
        ...
        case 'update': {
            ...
            if ($maKH === '' || ($trangThai !== 'Đã duyệt' && $trangThai !== 'Từ chối')) {
            ...
            $ok = $model->updateStatus($maKH, $trangThai, $ghiChu);
            if ($ok) {
                $hanhDong = ($trangThai === 'Đã duyệt') ? 'Duyệt kế hoạch mua sắm' : 'Từ chối kế hoạch mua sắm';
                $model->writeLog($_SESSION['maND'], $hanhDong, 'KeHoachMuaSam#'.$maKH);
